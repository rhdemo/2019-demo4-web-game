...
export function processFeedback (feedback: WSS.IncomingFrames.MotionFeedback) {
  ...
  if (localRecord) {
    if (localRecord.gesture === feedback.gesture) {
      ...
      setToastMessage(`Your ${feedback.gesture} was recognised with ${Math.round(feedback.probability)}% confidence!`)
    } else {
      setToastMessage(`You attempted a ${localRecord.gesture}, but it didn't quite work. Try again!`)
    }
  }
}
